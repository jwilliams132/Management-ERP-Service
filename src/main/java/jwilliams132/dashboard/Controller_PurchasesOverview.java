...
public class Controller_PurchasesOverview {
	...
	public void handleCategoryButtons(ActionEvent event) {
		...
		if (source == overall) {
			...
			updateValueLabel(
					calculateProfit(
							storageManager.getPurchasesAfterPointInTime(LocalDateTime.now().minusMonths(1))));
		} else if (source == pastSixMonths) {

			updateValueLabel(
					calculateProfit(
							storageManager.getPurchasesAfterPointInTime(LocalDateTime.now().minusMonths(6))));
		}
	}
	...
	public BigDecimal calculateProfit(List<Purchase> purchases) {

		BigDecimal profit = BigDecimal.ZERO;

		profit = purchases.stream()
				.map(purchase -> purchase.getPurchasePrice().negate())
